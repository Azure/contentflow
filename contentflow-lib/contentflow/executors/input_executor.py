...
class InputExecutor(BaseExecutor, ABC):
    ...
    async def crawl(
        ...
        """
        Crawl the content source and return a list of Content items.
        
        This method must be implemented by subclasses to define the
        specific crawling logic for the content source. It supports
        incremental crawling via checkpoints and pagination via continuation tokens.
        ...
        - Return a continuation_token if more results are available
        - Return None as continuation_token when all results are fetched
        ...
        async def crawl(self, checkpoint_timestamp=None, continuation_token=None):
            ...
            # Query with checkpoint and pagination
            items, next_token = await self.source.query(
            ...
            # Update checkpoint if this is the final page
        ...
        Args:
            ...
            continuation_token: Optional token for pagination. If provided, continues
                fetching from where the previous crawl left off.
        
        Returns:
            A tuple containing:
            - List of Content items retrieved from the source (up to batch_size)
            - Optional continuation token for next page, or None if complete
        """
