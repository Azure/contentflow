...
import { ExecutorNode } from "@/components/workflow/ExecutorNode";
import { SubWorkflowNode } from "@/components/workflow/SubWorkflowNode";
import { ExecutorConfigDialog } from "@/components/workflow/ExecutorConfigDialog";
import { PipelineSaveDialog } from "@/components/workflow/PipelineSaveDialog";
import { SavedPipelinesDialog, SavedPipeline } from "@/components/workflow/SavedPipelinesDialog";
...
export const WorkflowBuilder = () => {
  ...
  const addExecutorNode = (executor: ExecutorType, config?: any) => {
    const newNode: Node = {
      ...
      type: executor.category === "workflow" ? "subworkflow" : "executor",
        ...
        setNodes((nds) =>
          nds.map((node) =>
            node.id === selectedNode.id
              ? { ...node, data: { ...node.data, label: config.name, config } }
              : node
          )
        );
  ...
  const runWorkflow = () => {
    if (nodes.length === 0) {
      toast.error("Add at least one executor to run the workflow");
      return;
    }
  ...
  const getCategoryIcon = (category: string) => {
    const icons: Record<string, React.ReactNode> = {
      ...
      workflow: <Network className="w-4 h-4" />
    };
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="font-display text-4xl font-bold mb-2 text-foreground">
                {currentPipelineName || "Workflow Builder"}
              </h1>
              <p className="text-muted-foreground">
                {currentPipelineDescription || "Design complex processing pipelines with sub-workflows"}
              </p>
            </div>
                    ...
                    <CollapsibleContent className="mt-1 space-y-1">
                      {displayExecutors.map((executor) => (
                        <Button
                          key={executor.id}
                          variant="outline"
                          className="w-full justify-start gap-2 h-auto py-1.5 px-2 hover:shadow-sm transition-all text-left"
